...
% meshgrid commad, converting these to polar coordinates and finally
% defining the spiral texture
[x, y] = meshgrid(-120:1:120, -120:1:120);
% https://ww2.mathworks.cn/help/matlab/ref/cart2pol.html
[th, r] = cart2pol(x, y);
spiral = grey + inc .* cos(r / 5 + th * 5);

% Make our spiral texture into a screen texture for drawing
...
% Define the desination rectangles for our spiral textures. For this demo
% these will be the same size as out actually texture, but this doesn't
% have to be the case. See: ScaleSpiralTextureDemo and CheckerboardTextureDemo.
[s1, s2] = size(x);
baseRect = [0 0 s1 s2];
dstRects = nan(4, 4);
for i = 1:4
